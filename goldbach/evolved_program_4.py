...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    p_opt = 0.70 - 0.40 * factor
    p_add = 0.20 + 0.25 * factor
    p_drop = 0.10 + 0.15 * factor
    ...
    if action == 'add':
        cand_pool = [m for m in cand_mods if m not in congr_dict]
        if cand_pool:
            sample_size = min(250, len(cand_pool))
            # Bias sampling towards smaller primes as they have higher coverage density
            cand_pool_sorted = sorted(cand_pool)
            core_size = min(50, len(cand_pool_sorted))
            sampled_cands = set(cand_pool_sorted[:core_size])
            rem_pool = cand_pool_sorted[core_size:]
            if rem_pool:
                rem_sample = min(sample_size - core_size, len(rem_pool))
                sampled_cands.update(random.sample(rem_pool, rem_sample))
            sampled_cands = list(sampled_cands)
            ...
            for r in sampled_cands:
                if len(uncovered_targets) > 0:
                    ...
                    best_as = np.where(counts == max_count)[0]
                    a = int(random.choice(best_as))
                    k = max_count
                else:
                    a = 0
                    k = 0
        ...
        if mods_in:
            ...
            # We perform up to 3 Gibbs sampling sweeps on different moduli to accelerate convergence.
            num_sweeps = min(3, len(mods_in))
            local_cnt_array = cnt_array.copy()

            for sweep in range(num_sweeps):
                sweep_mods_in = list(congr_dict.keys())
                sample_size = min(40, len(sweep_mods_in))
                sample_opt = random.sample(sweep_mods_in, sample_size)
                ...
                for r in sample_opt:
                    ...
                    cnt_bg = local_cnt_array - is_covered_by_r
                    ...
                    if len(uncovered_targets) > 0:
                        ...
                        max_count = np.max(counts)
                        best_as = np.where(counts == max_count)[0]
                        a_new = int(random.choice(best_as))
                        k_new = max_count
                    else:
                        a_new = 0
                        k_new = 0
                    ...
                    k_old = np.sum((local_cnt_array == 1) & is_covered_by_r)
                    net_gain = k_new - k_old

                    scores.append(net_gain)
                    opts_data.append((r, a_new, is_covered_by_r))
                ...
                r_opt, a_new, old_cover_mask = opts_data[idx]

                a_old = congr_dict[r_opt]
                if a_new != a_old:
                    congr_dict[r_opt] = int(a_new)
                    new_cover_mask = (targets % r_opt) == a_new
                    local_cnt_array = local_cnt_array - old_cover_mask + new_cover_mask
