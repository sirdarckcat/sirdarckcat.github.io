...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    # 1. Run coordinate descent once at the start to ensure optimal residues
    ...
    # 2. Multi-step inner simulated annealing loop to find much deeper local minima
    num_steps = 45
    temp_decay = 0.90

    for step in range(num_steps):
        # Periodically align/optimize residues to prevent drift
        ...
        # Generate candidate moves: ADD, DROP, noop
        ...
        if cand_pool:
            ...
            if len(cand_pool) <= 40:
                ...
                sampled_adds = cand_pool[:20] + random.sample(cand_pool[20:], 20)
            ...
            if len(uncovered) > 0:
                ...
                for r in sampled_adds:
                    logm_new = logm + math.log(r)
                    logboost_new = logboost + math.log(r / (r - 1))
                    D_new = compute_D(logm_new, logboost_new, n_res)
                    moves.append(('add', r, 0, D_new))
        ...
        curr_temp = max(t_temp * (temp_decay ** step), 1e-4)
    ...
    # 3. Final coordinate descent to fully optimize residues of the final set
    for r in list(congr_dict.keys()):
        a_old = congr_dict[r]
        cnt_array -= (targets % r) == a_old
        uncovered = targets[cnt_array == 0]
