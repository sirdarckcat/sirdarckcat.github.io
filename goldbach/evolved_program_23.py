...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    if action == 'add':
        ...
        if cand_pool:
            ...
            # Standardize scores for scale-invariant thermodynamic sampling
            std_D = np.std(D_news) + 1e-8
            scaled_scores = -(D_news - np.mean(D_news)) / (std_D * temp)
            scaled_scores = scaled_scores - np.max(scaled_scores)
            probs = np.exp(scaled_scores)

            # Tensor Decomposition Rank-K update: Add multiple candidates at once
            num_add = min(3, len(cand_pool))
            probs = probs + 1e-8
            probs /= np.sum(probs)
            indices = np.random.choice(len(cand_pool), size=num_add, replace=False, p=probs)
            for idx in indices:
        ...
        if mods_in:
            ...
            temp = max(t_temp, 1e-4)
            std_D = np.std(all_D_news) + 1e-8
            scaled_scores = -(all_D_news - np.mean(all_D_news)) / (std_D * temp)
            scaled_scores = scaled_scores - np.max(scaled_scores)
            probs = np.exp(scaled_scores)
            probs /= np.sum(probs)
    ...
    # Tensor-Decomposition Inspired Alternating Least Squares (ALS)
    # Perform coordinate descent on the residues holding moduli fixed
    mods_in = list(congr_dict.keys())
    if mods_in:
        cnt_array = np.zeros(len(targets), dtype=np.int16)
        for r_in, a_in in congr_dict.items():
            cnt_array += (targets % r_in) == a_in

        changed = True
        als_iter = 0
        while changed and als_iter < 3:
            changed = False
            np.random.shuffle(mods_in)
            for r in mods_in:
                a_old = congr_dict[r]
                is_covered = (targets % r) == a_old
                cnt_array -= is_covered

                uncovered_bg = targets[cnt_array == 0]
                if len(uncovered_bg) > 0:
                    counts = np.bincount(uncovered_bg % r, minlength=r)
                    a_best = int(np.argmax(counts))
                else:
                    a_best = a_old

                if a_best != a_old:
                    congr_dict[r] = a_best
                    is_covered = (targets % r) == a_best
                    changed = True
