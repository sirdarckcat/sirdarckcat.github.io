...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    for inner in range(12):
        ...
        # 2. Block-randomized Coordinate descent for robust local optimization
        for cd_pass in range(2):
            ...
            for r in keys:
                ...
                if len(uncovered) > 0:
                    ...
                    max_c = counts.max()
                    best_a = np.where(counts == max_c)[0]
                    a_new = int(np.random.choice(best_a))
                else:
                    a_new = 0
                congr_dict[r] = a_new
        ...
        if active_primes and sampled_adds:
            sampled_drops = active_primes if len(active_primes) <= 20 else random.sample(active_primes, 20)
            sampled_swap_adds = sampled_adds if len(sampled_adds) <= 20 else random.sample(sampled_adds, 20)

            for r_drop in sampled_drops:
                cnt_array_drop = cnt_array - ((targets % r_drop) == congr_dict[r_drop])
                uncov = targets[cnt_array_drop == 0]

                for r_add in sampled_swap_adds:
                    if len(uncov) > 0:
                        counts = np.bincount(uncov % r_add, minlength=r_add)
