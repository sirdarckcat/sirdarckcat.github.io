...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    if action == 'add':
        ...
        if cand_pool:
            ...
            # Thermodynamic Field: Assign "Fugacity Weights" to all targets.
            # Uncovered targets get max weight, covered ones get exponentially decayed weight.
            # This builds "coverage momentum" and redundancy (Deep Cover).
            T_field = max(t_temp, 0.05)
            W_field = np.exp(-cnt_array / T_field)

            for r in sampled_cands:
                # Calculate fuzzy counts over all targets
                fuzzy_counts = np.bincount(targets % r, weights=W_field, minlength=r)

                # Introduce Quantum-like Gumbel noise to the fuzzy counts for exploration
                noise = -np.log(-np.log(np.random.uniform(1e-10, 1.0, size=r)))
                fuzzy_counts += noise * T_field * 0.5

                a = int(np.argmax(fuzzy_counts))

                # Actual uncovered targets covered by this choice
                k = np.sum((targets % r == a) & (cnt_array == 0))
        ...
        if mods_in:
            ...
            for r in sample_opt:
                ...
                # Entangled Thermodynamic Field
                W_field = np.exp(-cnt_bg / T_field)
                fuzzy_counts = np.bincount(targets % r, weights=W_field, minlength=r)

                # Quantum fluctuation (Gumbel noise)
                noise = -np.log(-np.log(np.random.uniform(1e-10, 1.0, size=r)))
                fuzzy_counts += noise * T_field * 0.5

                a_new = int(np.argmax(fuzzy_counts))

                # Calculate actual net gain based on hard counts for the evaluation score
                k_new = np.sum((targets % r == a_new) & (cnt_bg == 0))
                k_old = np.sum((targets % r == a_old) & (cnt_bg == 0))

                # Net gain in terms of D-score approximation
        ...
        if mods_in:
            ...
            for r in mods_in:
                a = congr_dict[r]
                is_covered = ((targets % r) == a)
                # Elements that become uncovered if we drop r
                k = np.sum((cnt_array == 1) & is_covered)

                # Thermodynamic "Ghost" Field: congestion relief
                # We slightly favor dropping primes that cover densely covered targets
                relief = np.sum(cnt_array[is_covered] - 1) * 0.01 * t_temp
                ...
                # Add congestion relief as a thermodynamic bonus
                scores.append(-D + relief)
