    ...
    for inner in range(15):
        ...
        for r in active_primes:
            ...
            if len(uncovered) > 0:
                ...
                curr_temp_cd = max(t_temp * (0.85 ** inner), 1e-4)
                if curr_temp_cd > 1e-3:
                    logits = counts * (0.05 / curr_temp_cd)
                    # To prevent floating point issues in exp, clip lower bound
                    logits = np.maximum(logits, np.max(logits) - 20.0)
                    logits -= np.max(logits)
                    probs = np.exp(logits)
                    probs /= np.sum(probs)
                    a_new = int(np.random.choice(r, p=probs))
                else:
        ...
        if cand_pool:
            cand_pool.sort()
            if len(cand_pool) <= 120:
                sampled_adds = cand_pool
            else:
                # Heavy sampling of small primes, decent sampling of larger primes
                sampled_adds = cand_pool[:40] + random.sample(cand_pool[40:], 80)
        ...
        for r, a in congr_dict.items():
            ...
            drop_info.append((r, a, k_drop, logm_new, logboost_new))

        # Evaluate a few Swap moves to bypass energy barriers
        if drop_info and sampled_adds:
            # Pick up to 5 least useful primes to try swapping out
            drop_info.sort(key=lambda x: x[2])
            for r_drop, a_drop, k_drop, lm_drop, lb_drop in drop_info[:5]:
                # Temporarily update uncovered for the dropped prime
                temp_uncovered = targets[(cnt_array == 0) | ((cnt_array == 1) & ((targets % r_drop) == a_drop))]
                swap_cands = random.sample(sampled_adds, min(len(sampled_adds), 5))
                for r_add in swap_cands:
                    if len(temp_uncovered) > 0:
                        counts = np.bincount(temp_uncovered % r_add, minlength=r_add)
                        a_add = int(np.argmax(counts))
                        k_add = counts[a_add]
                    else:
                        a_add = 0
                        k_add = 0
                    n_res_swap = len(temp_uncovered) - k_add
                    lm_swap = lm_drop + math.log(r_add)
                    lb_swap = lb_drop + math.log(r_add / (r_add - 1))
                    D_swap = compute_D(lm_swap, lb_swap, n_res_swap)
                    moves.append(('swap', (r_drop, r_add), a_add, D_swap))
