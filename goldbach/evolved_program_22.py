...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    LN10 = math.log(10)
    C_coeff = 4.0 / (LN10 * LN10)
    n_res_exact = float(np.sum(cnt_array == 0))
    m_digits = logm / LN10
    boost = math.exp(logboost)
    U_val = n_res_exact * boost
    sqrt_term = math.sqrt(m_digits**2 + C_coeff * U_val) if (m_digits**2 + C_coeff * U_val) > 0 else 1e-9

    # Exact partial derivatives of the D metric
    dD_dM = 0.5 * (1.0 + m_digits / sqrt_term)
    dD_dU = 0.5 * C_coeff / sqrt_term
    ...
    if action == 'add':
        cand_pool = [m for m in cand_mods if m not in congr_dict]
        if cand_pool:
            # Quantum Multi-verse evaluation: test many moduli simultaneously
            sample_size = min(100, len(cand_pool))
            sampled_cands = random.sample(cand_pool, sample_size)

            # Fractional Entanglement: evaluate non-linear topology of covered targets
            weights_add = np.exp(-1.2 * cnt_array)
            ...
            for r in sampled_cands:
                ...
                # Consider top 3 residues for adding
                top_k = min(3, r)
                best_as = np.argpartition(counts, -top_k)[-top_k:]
                for a in best_as:
                    K = counts[a]
                    dU = boost * (n_res_exact / (r - 1) - K * r / (r - 1))
                    dM = math.log(r) / LN10
                    dD = dD_dM * dM + dD_dU * dU
                    dD_list.append(dD)
                    cands_data.append((r, a))

            # Convert to thermodynamic energy scores (lower dD is better)
            scores = -np.array(dD_list)
        ...
        if mods_in:
            # Multi-dimensional topology sweep: re-evaluate active moduli assignments
            sample_size = min(12, len(mods_in))
            ...
            for r in sampled_mods:
                a_old = congr_dict[r]
                rem = targets % r
                is_covered_by_r = (rem == a_old)
                cnt_bg = cnt_array - is_covered_by_r

                # Fractional entanglement weights based on background topology
                weights_bg = np.exp(-1.2 * cnt_bg)
                K_old = np.sum(weights_bg[is_covered_by_r])

                counts = np.bincount(rem, weights=weights_bg, minlength=r)
                ...
                for a_new in best_as:
                    if a_new == a_old:
                        ...
                        K_new = counts[a_new]
                        dU = boost * (K_old - K_new)
                        dD = dD_dU * dU
                    dD_list.append(dD)
                    cands_data.append((r, a_new))

            scores = -np.array(dD_list)
            temp = max(t_temp, 1e-4) * 0.2
            scaled_scores = scores / temp
            scaled_scores = scaled_scores - np.max(scaled_scores)
            probs = np.exp(scaled_scores)
            probs /= np.sum(probs)
        ...
        mods_in = list(congr_dict.keys())
        if mods_in:
            # Calculate thermodynamic loss for removing each modulus
            dD_list = []
            for r in mods_in:
                a = congr_dict[r]
                mask = (targets % r) == a
                cnt_bg = cnt_array - mask

                # Using the exact same background weight definition
                weights_bg = np.exp(-1.2 * cnt_bg)
                K_lost = np.sum(weights_bg[mask])

                dU = boost * (K_lost * (r - 1) / r - n_res_exact / r)
                dM = -math.log(r) / LN10
                dD = dD_dM * dM + dD_dU * dU
                dD_list.append(dD)
