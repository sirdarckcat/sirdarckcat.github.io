...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    for inner in range(12):
        ...
        # 2. Coordinate descent to optimize residues of all active primes deterministically
        ...
        for r in keys:
            ...
            if len(uncovered) > 0:
                ...
                best_as = np.where(counts == max_cnt)[0]
                a_new = int(np.random.choice(best_as))
            else:
                a_new = 0
            congr_dict[r] = a_new
        ...
        if cand_pool:
            ...
            if len(cand_pool) <= 120:
                sampled_adds = list(cand_pool)
            else:
                sampled_adds = cand_pool[:40]
                mid = cand_pool[40:300]
                if mid: sampled_adds += random.sample(mid, min(40, len(mid)))
                tail = cand_pool[300:]
                if tail: sampled_adds += random.sample(tail, min(40, len(tail)))
        ...
        if congr_dict and sampled_adds:
            drop_cands = random.sample(list(congr_dict.keys()), min(15, len(congr_dict)))
            add_cands = random.sample(sampled_adds, min(15, len(sampled_adds)))
            for r_drop in drop_cands:
                a_drop = congr_dict[r_drop]
                mask_drop = (cnt_array == 1) & ((targets % r_drop) == a_drop)
                k_drop = np.sum(mask_drop)
                uncovered_if_dropped = targets[(cnt_array == 0) | mask_drop]
                for r_add in add_cands:
                    if len(uncovered_if_dropped) > 0:
                        counts = np.bincount(uncovered_if_dropped % r_add, minlength=r_add)
                        max_cnt = np.max(counts)
                        best_as = np.where(counts == max_cnt)[0]
                        a_add = int(np.random.choice(best_as))
                        k_add = counts[a_add]
                    else:
                        a_add = 0
                        k_add = 0
                    n_res_new = n_res + k_drop - k_add
                    logm_new = logm - math.log(r_drop) + math.log(r_add)
                    logboost_new = logboost - math.log(r_drop / (r_drop - 1)) + math.log(r_add / (r_add - 1))
                    D_new = compute_D(logm_new, logboost_new, n_res_new)
