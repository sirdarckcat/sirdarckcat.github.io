...
def optimize_cover_step(congr_dict, cand_mods, targets, cnt_array, logm, logboost, t_temp):
    ...
    for inner in range(25):
        ...
        # We use a tensor-inspired relaxation: we heavily weight uncovered targets (1.0),
        # but also give small positive weight (0.15) to targets covered exactly once.
        # This builds "redundancy" for fragile covers, enabling subsequent Drop moves.
        active_mods = list(congr_dict.keys())
        random.shuffle(active_mods)
        for r in active_mods:
            a_old = congr_dict[r]
            ...
            active_idx = np.nonzero(w > 0)[0]
            if len(active_idx) > 0:
                scores = np.bincount(targets[active_idx] % r, weights=w[active_idx], minlength=r)
                a_new = int(np.argmax(scores))
            else:
                a_new = 0
        ...
        if cand_pool:
            ...
            for r in sampled_adds:
                if len(active_idx) > 0:
                    ...
                    # Exact count of uncovered targets covered by choosing a
                    k = np.sum((uncovered % r) == a)
                else:
                    a = 0
                    k = 0
                ...
                # To encourage moves that provide redundancy, we slightly modify the effective D_new
                # in the Boltzmann probability by subtracting a redundancy bonus.
                redundancy_gain = max(0.0, float(scores[a] - k) / 0.15) if len(active_idx) > 0 else 0.0
                bonus = 0.005 * redundancy_gain * (curr_temp / 0.02)

                moves.append(('add', r, a, D_new - bonus))

        single_cov = targets[cnt_array == 1]
        for r, a in congr_dict.items():
            k_drop = np.sum((single_cov % r) == a)
